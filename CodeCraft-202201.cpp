...
string file_output = "solution.txt";
...
int ChooseNode(vector<int>& DD)
{
	int Index = distance( DD.begin(),min_element(DD.begin(), DD.end()));
	return Index + 1;
}
...
void AverageChoose(vector<vector<int>>& allc, int user, int node, bool av,vector<int> & NodeCount)
{
	if (user < allc.size())
	{
	...
	if (allc[user][0] != 0)
	{
		for (int j = node; j < allc[user].size(); j++)
		{
			if (allc[user][j] >= 0)
			{
				...
				//对该用户分配
				AverageChoose(allc, user, j, av, NodeCount);
				if (allc[user][0] == 0)
					break;
			}
		}
	...
	{
		AverageChoose(allc, user + 1, 1, av, NodeCount);
		return;
	}
}
...
//对节点最大值初始化
void InitMaxCount(vector<vector<int>>& CountJu,int node)
{
	for (int i = 1; i < CountJu.size(); i++)
	{
		//如果节点可分配带宽大于0 && 节点为联通
...
//单次计算开销 
int JudgePay(vector<vector<int>>& Time_Paid)
{
	static int Paid = 0;
	for (int i = 0; i < Time_Paid[0].size(); i++)
	{
		vector<int> This_node;
		for (int j = 0; j < Time_Paid.size(); j++)
		{
			This_node.push_back(Time_Paid[j][i]);
		}
		sort(This_node.begin(), This_node.end());
		Paid += This_node[(This_node.size() * 0.96)];
	}
	return Paid;
}
//时间节点上的分配 
void DealOneAlg(int Min_time, int Max_time, UserManage* Um, NodeManage* Nm, bool av, ofstream& outfile)
{
	...
	for (int i = Min_time; i < Max_time; i++)
	{
		...
		if (av)
			...
			InitMaxCount(CountJuTemp, node_number);
		int Reuslt = 1;
		...
		while (Reuslt != -1)
		{
			AverageChoose(CountJuTemp, Reuslt, node_number, av, Nm->Node_count);
			// if(!av)
			// {
		...
		//将开销放入数组
		for (int k = 1; k < CountJuTemp.size(); k++)
		{
			int width = CountJuTemp[0][k];
			Nm->Node_Pay[i][k - 1] = Nm->GetWidth(Nodenames[k - 1]) - width;
		}
...
int main()
{
	...
	//初始化不同时刻开销
	Nm->Node_Pay.resize(Max_times);
	for (int i = 0; i < Max_times; i++)
	{
		Nm->Node_Pay[i].resize(Nm->Get_Nodenames().size());
	}
	...
	DealOneAlg(Min_time, Max_times, Um, Nm, false, outfile);
	...
	//获得开销
	cout<<"score:" <<JudgePay(Nm->Node_Pay);
	ofstream Kfile("pay_data.txt");
	for (int i = 0; i < Nm->Node_Pay.size(); i++)
	{
		for (int j = 0; j < Nm->Node_Pay[i].size(); j++)
		{
			Kfile << "  Nodename:" << Nm->Get_Nodenames()[j] << "  " << Nm->Node_Pay[i][j];
		}
		Kfile << endl;
	}
	return 0;
